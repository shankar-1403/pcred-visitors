...
function ShellChrome({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-navy-500/10 bg-white/90 backdrop-blur-xl">
              ...
              <Link
                ...
                className={`flex min-h-10 items-center rounded-lg px-4 text-sm font-medium transition-colors duration-200 ${
                  pathname === item.href
                    ...
                    : "text-navy-500 hover:bg-navy-500/8"
                }`}
              >
                {item.label}
              </Link>
          ...
          <div className="flex items-center gap-2">
            {!isAdmin && pending.length > 0 ? (
              <Link
                ...
                className="flex min-h-10 items-center gap-2 rounded-lg bg-amber-100 px-3 text-xs font-semibold text-amber-800 transition-colors hover:bg-amber-200"
              >
            ...
            <span className="hidden max-w-40 truncate text-sm text-stone-600 lg:block">
            ...
            <button
              ...
              aria-label="Sign out"
              className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border border-navy-500/20 px-3 text-sm font-medium text-navy-500 transition-colors hover:bg-navy-500/8"
            >
            ...
            <button
              ...
              className="flex size-10 cursor-pointer items-center justify-center rounded-lg border border-navy-500/20 text-navy-500 sm:hidden"
            >
        ...
        {menuOpen && nav.length > 0 ? (
          <nav
            aria-label="Mobile"
            className="border-t border-navy-500/10 bg-white px-4 py-3 sm:hidden"
          >
            {nav.map((item) => (
              <Link
                key={item.href}
                ...
                className="flex min-h-12 items-center rounded-lg px-3 text-sm font-medium text-navy-500 hover:bg-navy-500/8"
              >
                {item.label}
              </Link>
            ))}
